use miniextendr_api::miniextendr;

use anyhow::{bail, Context};
use fs_err as fs;
use std::io::Write;
use std::path::Path;

// pharos config crate
use config::{CONFIG_FILENAME, Config};

/// Initializes pharos
///
/// @param config_path path to where pharos.toml is saved (should be colocated to where pharos is
/// run from)
///
/// @return nothing
/// @export
///
/// @examples \dontrun{
/// init("model/nonmem/submission-log/pharos.toml")
/// }
#[miniextendr]
pub fn init(config_path: &str) -> Result<(), anyhow::Error> {
    let path = Path::new(config_path);

    let config_path = if path.is_dir() {
        path.join(CONFIG_FILENAME)
    } else if path.file_name() == Some(std::ffi::OsStr::new(CONFIG_FILENAME)) {
        path.to_path_buf()
    } else {
        path.with_file_name(CONFIG_FILENAME)
    };

    if config_path.exists() {
        bail!("pharos config file already exists");
    }

    // Create parent directories if they don't exist
    if let Some(parent) = config_path.parent() {
        fs::create_dir_all(parent).context("Failed to create config directory")?;
    }

    let mut config_file =
        fs::File::create(&config_path).context("Failed to create config file")?;

    let nonmem_config =
        Config::new_nonmem().context("Failed to create pharos config")?;

    let config = toml::to_string_pretty(&nonmem_config).context("Failed to serialize config")?;

    config_file
        .write_all(config.as_bytes())
        .context("Failed to write config file")?;

    Ok(())
}
