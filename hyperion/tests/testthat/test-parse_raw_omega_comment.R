test_that("parse omega comments builds composite names", {
  # Composite name: prefix-theta_ref
  om_comment <- parse_raw_omega_comment("OMEGA(2,1)", NULL, "OM2,1 CL-VC")
  expect_equal(om_comment@nonmem_name, "OMEGA(2,1)")
  expect_equal(om_comment@name, "OM2,1-CL-VC")
...
test_that("omega name construction joins prefix and theta with hyphen", {
  # Already hyphenated - keeps as-is
  result <- extract_raw_omega_parts("IIV-CL")
  expect_equal(result$name, "IIV-CL")
  expect_equal(result$associated_theta, "CL")

  # Space between prefix and theta - joins with hyphen

  result <- extract_raw_omega_parts("IIV CL")
  expect_equal(result$name, "IIV-CL")
  expect_equal(result$associated_theta, "CL")

  # Linking word "on" - skipped, joins with hyphen
...
test_that("associated_theta splits unless matches known theta", {
  ...
  expect_equal(result$name, "IIV-CL/F")
