...
#' @keywords internal
#' @noRd
generate_omega_names <- function(structure_info, start_eta, block_size, num_params) {
  param_names <- c()

  if (structure_info == "Diagonal") {
    # Diagonal: each parameter gets (i,i)
    for (i in seq_len(num_params)) {
      param_names <- c(param_names, paste0("OMEGA(", start_eta + i - 1, ",", start_eta + i - 1, ")"))
    }
    ...
    for (col in 0:(block_size - 1)) {
      for (row in col:(block_size - 1)) {
        ...
        if (param_count <= num_params) {
          param_names <- c(param_names, paste0("OMEGA(", start_eta + row, ",", start_eta + col, ")"))
        }
      }
...
#' @keywords internal
#' @noRd
generate_sigma_names <- function(structure_info, start_eps, block_size, num_params) {
  param_names <- c()

  if (structure_info == "Diagonal") {
    # Diagonal: each parameter gets (i,i)
    for (i in seq_len(num_params)) {
      param_names <- c(param_names, paste0("SIGMA(", start_eps + i - 1, ",", start_eps + i - 1, ")"))
    }
  } else if (grepl("^Block", structure_info)) {
    # Block(n): lower triangular matrix, filled column-wise
    param_count <- 0
    for (col in 0:(block_size - 1)) {
      for (row in col:(block_size - 1)) {
        param_count <- param_count + 1
        if (param_count <= num_params) {
          param_names <- c(param_names, paste0("SIGMA(", start_eps + row, ",", start_eps + col, ")"))
        }
      }
      if (param_count >= num_params) break
    }
  ...
  if (requireNamespace("cli", quietly = TRUE)) {
    ...
    for (col in names(display_df)) {
      if (is.numeric(display_df[[col]])) {
        display_df[[col]] <- sprintf(paste0("%.", digits, "f"), display_df[[col]])
      }
    }
    ...
    for (i in seq_len(nrow(display_df))) {
      row_parts <- sapply(seq_len(ncol(display_df)), function(j) {
        ...
        if (col_name == "Parameter" && grepl("^(THETA|OMEGA|SIGMA|ETA|EPS)", cell_data)) {
        ...
        } else if ((col_name == "Initial" || col_name == "Estimate") && grepl("^[0-9]", cell_data)) {
          # Estimates in green
          padded_cell <- cli::col_green(padded_cell)
        } else if ((col_name == "Lower" || col_name == "Upper") && !is.na(cell_data) && cell_data != "NA") {
          # Bounds in yellow
    ...
    formatted_table <- knitr::kable(param_data, format = "simple", digits = digits)
